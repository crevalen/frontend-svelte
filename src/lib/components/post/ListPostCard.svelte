...
<a href={`/${category?.slug}/${post.slug}`} class="group grid grid-cols-3 sm:grid-cols-4 gap-4 sm:gap-6 items-center">
  ...
  <div class="col-span-2 sm:col-span-3">
    ...
    {/if}
    <h2 class="text-xl lg:text-2xl font-bold text-gray-900 dark:text-gray-100 mt-2 group-hover:text-cyan-600 dark:hover:text-cyan-400 transition-colors">{post.title}</h2>
