<script lang="ts">
  ...
  // generate schema hanya jika di halaman pertama
  $: homepageSchemas = homepageData?.pagination.currentPage === 1
    ? buildHomepageSchema({
        popularPosts: homepageData.popularPosts,
        gridPosts: homepageData.gridPosts,
...
<svelte:head>
  ...
  {#each homepageSchemas as schema}
