...
let showCookieBanner = false;
  ...
  function showNotificationLogic() {
...
if (currentScrollY > lastScrollY && currentScrollY > 150) {
   ...
   onMount(() => {
    ...
    // --- PERUBAHAN LOGIKA MOUNT ---
    if (!localStorage.getItem('cookie_consent')) {
      showCookieBanner = true;
    } else {
      // Jika cookie sudah diatur, langsung cek untuk notifikasi
      showNotificationLogic();
    }
    // --- AKHIR PERUBAHAN ---
...
{#if showCookieBanner}
  <CookieBanner on:accept={handleAcceptCookies} on:reject={handleRejectCookies} />
{/if}
